...
def insert_document(content, conn, cursor, model, commit=True, silent=False):
    ...
    Returns (combined_results, semantic_results, bm25_results, stats)
    """
    ...
    # 🚨 FIX 1: Explicitly return the stats dictionary
    ...
    print(f"{cs.RED}Input cannot be empty.{cs.RESET}")
    return [], {} # Return empty list and empty stats

    get_elapsed = measure_time()

    try:
    # 🚨 FIX 2: Unpack 4 return values
    ...
    # 🚨 FIX 3: Return both results list and the stats dictionary
    ...
    # 🚨 FIX 5: Return both results list and the stats dictionary
    return results[:top_k], hybrid_stats
