...
TEMPLATE_DIR = os.path.join(BASE_DIR, "core", "frontend", "templates")
STATIC_DIR = os.path.join(BASE_DIR, "core", "frontend", "static")
...
def highlight_text(text: str, query: str) -> str:
    ...
    # 1. Escape the raw text to prevent nested HTML bugs
    ...
    pattern = "|".join(terms)
    # We use a lambda to preserve case of the original match
    return re.sub(f"({pattern})", r"<mark>\1</mark>", safe_text, flags=re.IGNORECASE)
...
def home():
    ...
    if query:
        ...
        try:
            ...
            from core.frontend.graphs.analyze import generate_query_graph
